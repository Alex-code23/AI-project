...
if __name__ == "__main__":
    # tiny smoke test
    texts = load_texts_from_path(r"c:/Users/alexa/Desktop/my_book.txt", split_mode="paragraph")
    ...
    # model = SimpleLLM(vocab_size=len(tokenizer.vocab), d_model=128, n_heads=4, d_ff=512, n_layers=6, n_experts=2, moe_k=1)
    # model.train_model(texts=texts, tokenizer=tokenizer, epochs=10, batch_size=16, lr=1e-4, seq_len=32, save_dir=r"c:/Users/alexa/Desktop/checkpoints", print_every=400, use_amp=False)

    # Load later
    model, vocab = SimpleLLM.load("checkpoints/simplellm.pt", vocab_size=len(tokenizer.vocab), d_model=128, n_heads=4, d_ff=512, n_layers=6, n_experts=2, moe_k=1)
